...
<div id="app">
  <v-app>
  <v-container>
    <h2 v-cloak class="text-xs-center">Listado de cargos</h2>
    <v-layout>
      <v-flex>
        <v-dialog v-model="dialog" max-width="500px">
          <v-btn v-cloak slot="activator" color="222222" dark class="mb-2">Añadir cargo</v-btn>
          <v-card>
            <v-card-title>
...
new Vue({
  ...
  methods: {
    ...
    save(){
      let data = new FormData();
      data.append('position_form',JSON.stringify(this.editedItem));
      if(this.$refs.form.validate())
      {
        if(this.editedItem.id == null)
        {
          ...
          .then(response => {
            swal('Excelente!','Cargo creado correctamente','success')
            .then(val => {
              this.load();
              this.dialog = false;
            })
        ...
        else
        {
          ...
          .then(response => {
            swal('Excelente!','Cargo actualizado correctamente','success')
            .then(val => {
              this.load();
              this.dialog = false;
            })
